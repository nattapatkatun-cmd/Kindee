...
**ทดสอบ:** เปิด browser ไปที่ `https://kindee.ojo0308.workers.dev/health`
— ถ้าได้ JSON ที่มี `"ok":true` พร้อมเลข `version` ตรงกับค่า `WORKER_VERSION`
ในไฟล์ `worker/worker.js` ของ repo แปลว่า deploy เวอร์ชันล่าสุดสำเร็จ
(และดูได้ด้วยว่า `gemini` / `firebase` / `quotaKV` เป็น `true` ครบไหม)
ถ้าได้ `{"error":"not found"}` หรืออย่างอื่น = worker บนเซิร์ฟเวอร์ยังเป็น
เวอร์ชันเก่า ต้องวางโค้ดใหม่แล้ว Deploy อีกรอบ

> ⚠️ อาการคลาสสิกเมื่อ worker บนเซิร์ฟเวอร์เป็นเวอร์ชันเก่า: **เครื่องเจ้าของ
> ใช้ AI ได้ปกติ แต่ user คนอื่นเจอ "Load failed"** — เพราะเครื่องเจ้าของมี
> API key สำรองในเครื่อง แอปเลย fallback ให้เงียบ ๆ ส่วนคนอื่นไม่มี key
> คำขอที่แนบ Authorization header จะถูก CORS preflight ของ worker เก่าปฏิเสธ
